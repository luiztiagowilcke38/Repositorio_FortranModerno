...
CONTAINS
  ...
  SUBROUTINE calcular_airy_completo(x, ai, bi, aid, bid)
    ...
    REAL(real64) :: abs_x, f, g, fd, gd, z, termo_exp, amp, fase
    ...
    ELSE
      ...
      IF (x > 0.0_real64) THEN
        termo_exp = EXP(z); amp = 1.0_real64 / (2.0_real64 * SQRT(PI) * x**0.25_real64)
      ...
      ELSE
        fase = z + PI/4.0_real64; amp = 1.0_real64 / (SQRT(PI) * abs_x**0.25_real64)
  ...
  SUBROUTINE avaliar_series_airy(x, f, g, fd, gd)
    REAL(real64), INTENT(IN) :: x; REAL(real64), INTENT(OUT) :: f, g, fd, gd
    REAL(real64) :: t, tf, tg; INTEGER :: k
    f=1.0; g=x; fd=0.0; gd=1.0; tf=1.0; tg=x; t=x**3
    DO k = 1, 100
      tf = tf * t / (3*k * (3*k-1)); f = f + tf; fd = fd + tf * (3*k)/x
      tg = tg * t / (3*k * (3*k+1)); g = g + tg; gd = gd + tg * (3*k+1)/x
      IF (ABS(tf) < 1D-18 .AND. ABS(tg) < 1D-18) EXIT
    END DO
  END SUBROUTINE avaliar_series_airy

  FUNCTION avaliar_hankel(z, tipo_ai) RESULT(res)
    REAL(real64), INTENT(IN) :: z; LOGICAL, INTENT(IN) :: tipo_ai; REAL(real64) :: res, t, s
    INTEGER :: k; s = 1.0; t = 1.0
    DO k = 1, 35
      t = t * (6*k-5)*(6*k-3)*(6*k-1) / (k * 216 * z)
      IF (.NOT. tipo_ai) t = -t
      s = s + t
      IF (ABS(t) < 1E-16) EXIT
    END DO
  ...
  FUNCTION avaliar_osc(z, tipo_s) RESULT(res)
    REAL(real64), INTENT(IN) :: z; LOGICAL, INTENT(IN) :: tipo_s; REAL(real64) :: res, t, s
    INTEGER :: k; s = 1.0; t = 1.0
    DO k = 1, 35
      t = -t * (6*k-5)*(6*k-3)*(6*k-1) * (6*k+1)*(6*k+3)*(6*k+5) / ( (2*k)*(2*k-1) * (216*z)**2 )
      s = s + t
      IF (ABS(t) < 1E-16) EXIT
    END DO
    IF (tipo_s) THEN; res = s; ELSE; res = 1.0/z; END IF
  END FUNCTION avaliar_osc

  FUNCTION avaliar_osc_deriv(z, tipo_s) RESULT(res)
    REAL(real64), INTENT(IN) :: z; LOGICAL, INTENT(IN) :: tipo_s; REAL(real64) :: res
    res = avaliar_osc(z, tipo_s) * (1.0 + 0.1/z)
